...
def total_loss(
    ...
    if use_boundary_loss:
        ...
        loss = loss + 0.4 * F.binary_cross_entropy_with_logits(boundary_logits, boundary)
        for scale in outputs.get("boundaries", []):
            scaled_target = F.interpolate(boundary, size=scale.shape[-2:], mode="nearest")
            loss = loss + 0.1 * F.binary_cross_entropy_with_logits(scale, scaled_target)
    ...
    if use_gate_loss:
        ...
        for gate in gates:
            ...
            loss = loss + 0.08 * F.binary_cross_entropy(gate.clamp(1e-4, 1 - 1e-4), scaled_boundary)
    if use_contour_loss:
